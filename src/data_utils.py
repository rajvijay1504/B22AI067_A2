...
def download_youtube_audio(
    ...
    If start_sec and duration_sec are provided, trims using ffmpeg post-download.
   ...
   #Find the downloaded file
    ...
    if not os.path.exists(tmp_wav):
       #yt-dlp may rename; find matching file
...
def extract_segment(
    ...
    """
    Extract a time segment from a WAV file without re-downloading.
    """
...
# 2.  N-gram corpus from subtitles
    ...
    with open(vtt_path, encoding="utf-8") as f:
        for line in f:
            ...
            if not line or line.startswith("WEBVTT") or "-->" in line:
                continue
           #Remove HTML tags like <00:00:01.234> or <c>...</c>
            line = re.sub(r"<[^>]+>", "", line)
            if line:
...
def build_ngram_corpus(url: str, output_corpus_path: str) -> str:
    """
    Download subtitles from YouTube and save as plain text corpus
...
# 3.  Audio augmentation for training data
...
def augment_audio(waveform: np.ndarray, sample_rate: int,
    ...
    """Chain augmentations for robustness."""
    aug = waveform.copy()
...
# 4.  Parallel corpus builder for Maithili (Task 2.2)
def save_maithili_parallel_corpus(path: str = "data/maithili_parallel.txt"):
    """
    Write the 500-word Maithili technical dictionary to a parallel corpus file
    suitable for fine-tuning an MT system.
    """
