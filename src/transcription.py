...
# 1.  N-gram Language Model (pure Python, no external KenLM required)
class NgramLM:
    """
    Smoothed N-gram LM trained on the speech-course syllabus text.
    Used to bias Whisper logits toward domain technical terms.
    """
    ...
    def score(self, word: str, context: tuple) -> float:
        ...
        # Walk from longest to unigram context
...
# 2.  Logit-Bias Processor for Whisper
class NGramLogitBiasProcessor:
    """
    Whisper LogitsProcessor that adds a positive bias to token IDs
    whose decoded string matches a high-scoring N-gram term.

    Compatible with Hugging Face `transformers` generate() interface.
    """
    ...
    def __call__(self, input_ids: torch.LongTensor,
        ...
        """Called by Whisper generate() at every decoding step."""
...
# 3.  Whisper Transcription with Constrained Decoding
...
def transcribe_segment(
    ...
    """
    Transcribe an audio file using Whisper with N-gram logit bias.

    Returns list of dicts:
        [{"text": "...", "start": float, "end": float, "language": "hi"/"en"}]
    """
