...
def spectral_subtraction(
    ...
    """Power-spectrum subtraction (Boll 1979). Returns denoised float32 array."""
    stft = np.array([
    ...
    # Subtract noise estimate; enforce spectral floor to prevent negatives
...
def denoise_deepfilter(waveform: torch.Tensor, sample_rate: int) -> torch.Tensor:
    """DeepFilterNet wrapper. Falls back to spectral subtraction if package unavailable."""
    try:
...
def vad_trim(waveform: np.ndarray, sample_rate: int = 16000,
    ...
    """Remove silence using WebRTC VAD. Falls back to librosa trim if unavailable."""
    try:
        import webrtcvad
...
def rms_normalize(waveform: np.ndarray, target_db: float = -23.0) -> np.ndarray:
    """EBU R128-style RMS loudness normalization."""
...
def preprocess_audio(
    ...
    """Full preprocessing: load, resample, mono, denoise, VAD trim, normalize, save."""
    ...
    if method == "deepfilter":
        ...
        print(f"[Denoising] Applying spectral subtraction  (alpha={over_subtraction}, beta={spectral_floor})")
...
def compute_snr(clean: np.ndarray, noisy: np.ndarray) -> float:
    """Compute signal-to-noise ratio in dB between two equal-length signals."""
